...
results = ratings.group_by(ratings.movieID) \
    .select(ratings.movieID, ratings.rating.count.alias('best_rating'))

sorted = results.order_by(results.best_rating.desc).fetch(10)

#Propojeni s nazvem filmu
joined = sorted.join(movies) \
    .where(sorted.movieID == movies.id) \
    .select(movies.name, sorted.best_rating) \
    .order_by(sorted.best_rating.desc)

#Nadefinovani vystupni tabulky
otd = TableDescriptor.for_connector('print') \
    .schema(Schema.new_builder()
            .column('movieName', DataTypes.STRING())
            .column('popularity', DataTypes.BIGINT())
            .build()) \
    .build()
